...
def try_decompress_meshopt_vertex(compressed_data, vertex_count, stride):
    """使用 C meshopt2.dll 解码顶点缓冲。失败返回 None。"""
...
def try_decompress_meshopt_index(compressed_data, index_count, index_size=2):
    """使用 C meshopt2.dll 解码索引缓冲。失败返回 None。"""
    lib = _load_meshopt_lib()
    if not lib:
...
def export_obj_filtered(result, output_dir, base_name):
    ...
    # 地形 patch 材质 (不同色区分 A/B 阵列，非地形块真正颜色)
    patch_colors = [
        (0.2, 0.6, 0.2), (0.3, 0.5, 0.3), (0.1, 0.7, 0.2), (0.4, 0.6, 0.1),
        (0.5, 0.5, 0.2), (0.2, 0.4, 0.4), (0.3, 0.6, 0.4), (0.4, 0.3, 0.3),
        (0.5, 0.7, 0.3), (0.6, 0.4, 0.2), (0.7, 0.5, 0.3), (0.1, 0.5, 0.5),
        (0.3, 0.7, 0.5), (0.6, 0.6, 0.3), (0.4, 0.5, 0.5), (0.5, 0.3, 0.4),
    ]
    get_material("geo0_patch_base", (0.25, 0.55, 0.25))
    ...
    with open(obj_path, 'w') as f:
        ...
        for i, t in enumerate(result['terrain']):
            ...
            if ib_raw and patches:
                ...
                write_vertices(f, all_verts)
                global_v_offset += len(all_verts)
                ...
                # Array C 材质与其他信息
                if c_entries:
                    # 按 material ID 统计 (IDA: byte0=matID, byte1=vtx_cnt, byte2=idx_cnt, skip 0 and 80)
                    mat_groups = {}
                ...
                write_object(f"Terrain_{i}", f"terrain_{i}", t['verts'], t['indices'])

        for i, s in enumerate(result['skirts']):
            write_object(f"Skirt_{i}", f"skirt_{i}", s['verts'], s['indices'])
        for i, w in enumerate(result['occluder']):
            write_object(f"Occluder_{i}", "occluder", w['verts'], w['indices'])
        ...
        if cloud and cloud.get('proxy_verts') and cloud.get('proxy_indices'):
            ...
            if cv and ci:
                ...
                for t in range(tri_count):
                    ...
                    if i0 >= n_verts or i1 >= n_verts or i2 >= n_verts: continue
                    f.write(f"f {i0+global_v_offset} {i2+global_v_offset} {i1+global_v_offset}\n")
                global_v_offset += n_verts
                f.write("\n")
                # print(f"  [+] Cloud Proxy: {len(cv)} verts, {tri_count} tris")
